...
/*
 * P-wave width, in samples - EXPERIMENTAL, and meaningfully less trustworthy
 * than rpeak_qrs_width_*() above. A P-wave is roughly 5-10x smaller in
 * amplitude than a QRS complex with a much gentler slope, so on real-world
 * (not clean MIT-BIH archive) signal it is easy to miss a real one or latch
 * onto a noise ripple instead. Unlike QRS width, this has NOT been validated
 * against any labeled dataset. Same contract as rpeak_qrs_width_samples()
 * (r_index = where R sits in the 256-sample window, pass RPEAK_BEAT_PRE for
 * windows from rpeak_extract_beat()/rpeak_extract_beat_ctx()), and returns
 * -1 far more readily on purpose - a missing number is honest, a fabricated
 * one is not. See rpeak.c for the algorithm and its caveats.
 */
int rpeak_p_wave_width_samples(const int16_t beat[256], int r_index);

/* Same as rpeak_p_wave_width_samples(), converted to milliseconds (rounded
 * to the nearest ms). -1 propagates unchanged. */
int rpeak_p_wave_width_ms(const int16_t beat[256], int r_index);

/* Diagnostic only: the raw peak magnitude (Q8.8, always >= 0) found in the
 * P-wave search zone, WITHOUT the P_MIN_AMPLITUDE gate the width functions
 * above apply - so you can see how close a rejected beat came instead of
 * only ever seeing pass ("Nms") or fail ("--"). Returns -1 if there wasn't
 * even room to search (see rpeak.c). Use this to tune P_MIN_AMPLITUDE
 * against real captured beats, not as a measurement to build on. */
